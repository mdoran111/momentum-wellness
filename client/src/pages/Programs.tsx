    ...
    <div className="pt-12 pb-24 min-h-screen bg-background">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-3xl mb-16">
          <h1 className="text-4xl md:text-5xl font-bold font-heading text-primary mb-6">Program Library</h1>
          <p className="text-xl text-muted-foreground">Your membership unlocks every program below. Filter by your goals and available equipment to find your perfect starting point.</p>
        </div>

        {/* Filters */}
        <div className="mb-12 space-y-6">
          <div>
            <h3 className="text-sm font-semibold text-primary uppercase tracking-wider mb-3">Goal</h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-primary uppercase tracking-wider mb-3">Equipment</h3>
            <div className="flex flex-wrap gap-2">
              {EQUIPMENT.map(eq => (
                <button
                  key={eq}
            ...
            filteredPrograms.map(program => (
              <Dialog key={program.id}>
                <DialogTrigger asChild>
                  <Card className="overflow-hidden border-border flex flex-col hover:shadow-lg transition-shadow cursor-pointer group h-full">
                    ...
                    <CardHeader className="flex-1">
                      <div className="flex gap-2 mb-2">
                        <span className="text-xs font-semibold uppercase tracking-wider text-primary bg-primary/10 px-2 py-1 rounded">{program.goal}</span>
                        <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground bg-secondary px-2 py-1 rounded">{program.equipment}</span>
                      </div>
                      <CardTitle>{program.title}</CardTitle>
                      <CardDescription className="line-clamp-2">{program.description}</CardDescription>
                    </CardHeader>
                    <CardFooter className="pt-4 border-t">
                      <span className="text-sm font-semibold text-primary flex items-center w-full justify-between">
                ...
                <DialogContent className="sm:max-w-[600px]">
                  <DialogHeader>
                    ...
                    <DialogTitle className="text-2xl font-heading">{program.title}</DialogTitle>
                    <DialogDescription className="text-base pt-2">
                      {program.description}
                    </DialogDescription>
                  </DialogHeader>
                  
                  <div className="grid grid-cols-2 gap-4 py-6 border-y my-4">
                    <div className="flex items-start gap-3">
                      <Target className="w-5 h-5 text-primary mt-0.5" />
                      <div>
                        <h4 className="text-sm font-bold">Who it's for</h4>
                        <p className="text-sm text-muted-foreground">{program.details.who}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Clock className="w-5 h-5 text-primary mt-0.5" />
                      <div>
                        <h4 className="text-sm font-bold">Time per session</h4>
                        <p className="text-sm text-muted-foreground">{program.details.time}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <CalendarDays className="w-5 h-5 text-primary mt-0.5" />
                      <div>
                        <h4 className="text-sm font-bold">Days per week</h4>
                        <p className="text-sm text-muted-foreground">{program.details.days}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Dumbbell className="w-5 h-5 text-primary mt-0.5" />
                      <div>
                        <h4 className="text-sm font-bold">What you'll achieve</h4>
                        <p className="text-sm text-muted-foreground">{program.details.achieve}</p>
                      </div>
                    </div>
                  </div>
        ...
        {/* Bottom CTA */}
        <div className="mt-24 p-12 bg-primary rounded-3xl text-center text-white">
          ...
          <a href="https://example.com/checkout" target="_blank" rel="noreferrer">
            <Button size="lg" className="rounded-full bg-white text-primary hover:bg-white/90 font-bold px-8 h-12">
              Start Membership
            </Button>
          </a>
        </div>
